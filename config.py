...
# Пороги голосования для генерации сигнала (УЛУЧШЕНО: повышены для лучшей фильтрации)
VOTE_THRESHOLD_TRENDING = 2      # Оставляем 2 (оптимально)
VOTE_THRESHOLD_RANGING = 5       # УЛУЧШЕНО: было 4, увеличено до 5 для лучшей фильтрации
VOTE_THRESHOLD_TRANSITIONING = 6 # УЛУЧШЕНО: было 5, увеличено до 6 для уменьшения ложных сигналов

# Минимальное количество пройденных фильтров для сигнала
MIN_FILTERS = 3  # Из 5 возможных фильтров (УЛУЧШЕНО: было 2, увеличено для лучшей фильтрации)
MIN_FILTERS_SELL = 2  # Для SELL сигналов (УЛУЧШЕНО: было 1, ужесточено для уменьшения убыточных SELL)
...
# Использовать Kelly Criterion для оптимального размера позиции (УЛУЧШЕНО: более консервативно)
USE_KELLY_CRITERION = True
KELLY_FRACTION = 0.12  # УЛУЧШЕНО: 12% Kelly для консервативности (было 15%, снижено для уменьшения риска)
...
# Стоп-лосс и тейк-профит (УЛУЧШЕНО: улучшен R:R для низкого win rate)
STOP_LOSS_PERCENT = 0.035  # 5% стоп-лосс (базовый)
TAKE_PROFIT_PERCENT = 0.07  # 7% тейк-профит (ИСПРАВЛЕНО - было 10%)
PARTIAL_CLOSE_PERCENT = 0.50  # Закрываем 50% позиции на TP
TRAILING_STOP_PERCENT = 0.015  # УЛУЧШЕНО: 1.5% trailing stop (было 2% - более агрессивный)
TRAILING_STOP_ACTIVATION = 0.01  # УЛУЧШЕНО: активировать после +1% прибыли

# Динамический стоп-лосс на основе ATR (УЛУЧШЕНО)
DYNAMIC_SL_ATR_MULTIPLIER = 4.5  # УЛУЧШЕНО: 4.5x ATR для расчета SL (было 4.0)
DYNAMIC_SL_MIN = 0.05  # Минимум 5% (оставляем)
DYNAMIC_SL_MAX = 0.15  # Максимум 15% (оставляем)

# Динамический тейк-профит на основе ATR (НОВОЕ: для Trend Following)
USE_DYNAMIC_TP_FOR_TF = True  # Использовать динамический TP на основе ATR для Trend Following
DYNAMIC_TP_ATR_MULTIPLIER = 5.0  # 5.0x ATR для TP (оптимальный R:R ~2.2:1 при SL 4.5x ATR)
DYNAMIC_TP_MIN = 0.08  # Минимум 8% TP
DYNAMIC_TP_MAX = 0.18  # Максимум 18% TP
...
# Break-even Stop Loss (УЛУЧШЕНО: более ранняя защита прибыли)
BREAKEVEN_PROFIT_THRESHOLD = 0.015  # УЛУЧШЕНО: при +1.5% прибыли (было 2% - более ранняя защита)
...
# Время удержания позиции (УЛУЧШЕНО: ускорен выход из позиций)
MAX_HOLDING_HOURS = 36  # УЛУЧШЕНО: максимум 36 часов (1.5 дня) для Trend Following (было 48)
# MR_MAX_HOLDING_HOURS задан ниже в секции Mean Reversion (строка 231)

# Partial Take Profit (УЛУЧШЕНО - отложено для лучшего R:R)
USE_PARTIAL_TP = True  # Включить частичное закрытие позиции
PARTIAL_TP_PERCENT = 0.5  # Закрывать 50% позиции
PARTIAL_TP_TRIGGER = 0.02  # УЛУЧШЕНО: На +2% прибыли (было 1%, отложено для лучшего R:R)
PARTIAL_TP_REMAINING_TP = 0.03  # УЛУЧШЕНО: TP для оставшихся 50%: +3% (было 2%)
...
# Bayesian Decision Layer (УЛУЧШЕНО: повышена минимальная вероятность)
BAYESIAN_MIN_PROBABILITY = 0.65  # УЛУЧШЕНО: минимальная вероятность успеха для входа (было 55%, стало 65%)
BAYESIAN_MIN_SAMPLES = 10  # Минимальное количество сигналов для надёжной статистики
ENFORCE_BAYESIAN_FILTER = True  # УЛУЧШЕНО: обязательный фильтр (если модель доступна)
...
# TP/SL для Mean Reversion v5 (УЛУЧШЕНО: улучшен R:R)
MR_TAKE_PROFIT_PERCENT = 0.05  # УЛУЧШЕНО: 5% тейк-профит (было 3.5%, R:R улучшен)
MR_STOP_LOSS_PERCENT = 0.035  # УЛУЧШЕНО: 3.5% стоп-лосс (было 2.8%, более реалистично)
MR_MAX_HOLDING_HOURS = 24  # УЛУЧШЕНО: Максимум 24 часа удержания (было 48h, сокращено для быстрого выхода)

# Z-score параметры для MR
MR_ZSCORE_WINDOW = 50  # Окно для расчёта среднего

# Фильтры "падающего ножа" v5.4 (УЛУЧШЕНО: ужесточены для защиты от убытков)
...
NO_BUY_IF_PRICE_BELOW_N_DAY_LOW_PERCENT = 0.05  # УЛУЧШЕНО: было 0.10, стало 0.05 (более строгий)
...
RED_CANDLES_THRESHOLD = 3  # УЛУЧШЕНО: блокировать при 3+ красных свечах подряд

# Фильтр объёма v5.3 (УЛУЧШЕНО: ужесточен)
USE_VOLUME_FILTER = True  # v5: Блокировать входы при всплесках объёма
VOLUME_SPIKE_THRESHOLD = 3.5  # УЛУЧШЕНО: Не входить если volume > 3.5x средний за 24h (было 3.0)

# Динамический SL на основе ATR v5 (УЛУЧШЕНО)
USE_DYNAMIC_SL_FOR_MR = True  # Использовать ATR-based SL вместо фиксированного
MR_ATR_SL_MULTIPLIER = 2.5  # УЛУЧШЕНО: Множитель ATR для SL (2.5x ATR, было 2.0)
MR_ATR_SL_MIN = 0.025  # УЛУЧШЕНО: Минимальный SL 2.5% (было 2%)
MR_ATR_SL_MAX = 0.045  # УЛУЧШЕНО: Максимальный SL 4.5% (было 4%)
...
# Динамический TP v5 (УЛУЧШЕНО)
USE_DYNAMIC_TP_FOR_MR = True  # Использовать динамический TP на основе ATR
MR_ATR_TP_MULTIPLIER = 3.5  # УЛУЧШЕНО: 3.5x ATR для TP (было 2.8, улучшен R:R)
MR_ATR_TP_MIN = 0.03  # УЛУЧШЕНО: Минимум 3% (было 2.5%)
MR_ATR_TP_MAX = 0.06  # УЛУЧШЕНО: Максимум 6% (было 4.5%)

# Минимальный R:R контроль (УЛУЧШЕНО: улучшен минимальный R:R)
MIN_RR_RATIO = 1.5  # УЛУЧШЕНО: Минимальный Risk:Reward ratio (1:1.5, было 1.25)
ENFORCE_MIN_RR = True  # Принудительно обеспечивать минимальный R:R

# Трейлинг стоп для MR v5 (УЛУЧШЕНО: более консервативный)
USE_TRAILING_STOP_MR = True  # Включить трейлинг стоп после прибыли
MR_TRAILING_ACTIVATION = 0.02  # УЛУЧШЕНО: Активация после +2% (было 1.5%, отложено для лучшего R:R)
MR_TRAILING_DISTANCE = 0.012  # УЛУЧШЕНО: 1.2% от максимума (было 1.0%, более консервативно)
...
# Кулдаун между сделками для одного символа (УЛУЧШЕНО: увеличен для уменьшения скальпинга)
TRADE_COOLDOWN_MINUTES = 60  # УЛУЧШЕНО: Минимум 60 минут между сделками (было 30, уменьшает скальпинг)

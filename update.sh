...
# Миграция базы данных (только для systemd, для Docker выполняется в контейнере)
if [ "$METHOD" = "systemd" ]; then
	echo "🔄 Проверка миграции базы данных..."
	if [ -f "migrate_database.py" ]; then
		...
		if [ $? -eq 0 ]; then
			...
			echo "⚠️  Миграция базы данных завершилась с предупреждениями"
		fi
	else
		echo "ℹ️  Миграция базы данных не найдена, пропускаем"
	fi
	echo ""
elif [ "$METHOD" = "docker" ]; then
	echo "ℹ️  Миграция базы данных будет выполнена в Docker контейнере"
	echo ""
fi
